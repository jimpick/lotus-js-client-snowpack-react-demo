...
((function(exports, React) {
  ...
  function addTrappedEventListener(targetContainer, topLevelType, capture, legacyFBSupport, passive, priority) {
    ...
    listener = listenerWrapper.bind(null, topLevelType, PLUGIN_EVENT_SYSTEM, targetContainer); // When the targetContainer is null, it means that the container
    ...
    var validTargetContainer = targetContainer;
    var rawEventName = getRawEventName(topLevelType);
    var fbListener; // When legacyFBSupport is enabled, it's for when we

    if (capture) {
      {
        fbListener = addEventCaptureListener(validTargetContainer, rawEventName, listener);
      }
    } else {
      {
        fbListener = addEventBubbleListener(validTargetContainer, rawEventName, listener);
      }
  ...
  function warnAboutUpdateOnUnmountedFiberInDEV(fiber) {
    {
      ...
      if (isFlushingPassiveEffects) {
        error("Can't perform a React state update from within a useEffect cleanup function. " + 'To fix, move state updates to the useEffect() body in %s.%s', tag === ClassComponent ? 'the componentWillUnmount method' : 'a useEffect cleanup function', getStackByFiberInDevAndProd(fiber));
      } else {
  ...
  var ReactVersion = '16.13.0-experimental-aae83a4b9';
...
const {__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED,createBlockingRoot,createPortal,createRoot,findDOMNode,flushSync,hydrate,render,unmountComponentAtNode,unstable_batchedUpdates,unstable_createPortal,unstable_discreteUpdates,unstable_flushControlled,unstable_flushDiscreteUpdates,unstable_renderSubtreeIntoContainer,unstable_scheduleHydration,version} = ReactDOM;
